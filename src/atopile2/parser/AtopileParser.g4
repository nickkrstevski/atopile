...
simple_stmt: import_stmt | assign_stmt | setas_stmt | connect_stmt;
compound_stmt: class_def;
block: simple_stmts | NEWLINE INDENT stmt+ DEDENT;

// in it's most tricked out form
// eg. block <name> from <some_block>, <another_block>:
class_def: class_type name ( 'from' class_ref ( ',' class_ref )* )? ':' block;

class_type: 'block' | 'node';

import_stmt: 'import' class_ref ( ',' class_ref )* 'from' string;

assign_stmt: refs ( ':' class_ref )? '=' instantiation | refs | value ;
setas_stmt: refs '->' class_ref;
connect_stmt: refs '~' refs;

refs: single_ref ( ',' single_ref )*;
single_ref: name ('[' (string | totally_an_integer) ']')? ('.' single_ref)?;

instantiation: 'new' class_ref;

class_ref: attr | name;
attr: name ('.' name)+;
value: string | number | physical | boolean_;
name: NAME;
string: STRING;
physical: STRING;
number: STRING;
